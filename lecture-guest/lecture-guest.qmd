...
::: {.w-60 .pl3 .fragment fragment-index=1 .ruby2}
...
::: {.w-20 .fragment fragment-index=1}
...
## How does plant diversity and warming effect on essential ecosystem properties, *such as soil microbial functions*? {auto-animate=true}
...
::: {.w-60 .pl3 .fragment fragment-index=1 .blue2}
...
<i class="fa-solid fa-comments"></i> **_Communication_** is the key to get the most out of the experimental data
...
<center style="font-size:1.2em;color: white">

```{r}
#| echo: false
#| output: asis
cat(fa_list(c("fas fa-code" = "Use code to formulate your understanding and for communication.","fas fa-lightbulb" = 'Combine limited number of "words" (functions) to construct a variety of "sentences" (experimental designs).'), incremental = TRUE))
```
...
- The aim of `edibble` package is to make an **e**xperimental **d**esign t**ibble** [(note: tidyverse version of a table)]{style="font-size:0.5em!important;"}.
...
- You can consider simulating data for your experiment.
...
```{r}
mysimprocess <- myplan %>% 
  simulate_process(biomass = function(deffect = 2,
                                      heffects = c(0, 1, 2)) {
    # assume the following effects
    diversity_effect <- deffect
    heat_effect <- c("0°C" = heffects[1], "1.5°C" = heffects[2], "3°C" = heffects[3])
    
    # let's assume that the data generating process is as follows:
    diversity_effect * diversity + heat_effect[heat] + rnorm(n(), 0, 1.5)
  }) 
```

- The return object needs to te the same size as the number of rows for the design table.

## Simulating the data 

```{r}
sim1 <- mysimprocess %>% 
  simulate_rcrds(biomass = with_params())

sim2 <- mysimprocess %>% 
  simulate_rcrds(biomass = with_params(deffect = 3, heffects = c(-1, 3, 1)))
```
...
```{r}
library(ggplot2)
g1 <- ggplot(sim1, aes(diversity, biomass, color = heat)) +
  geom_jitter() +
